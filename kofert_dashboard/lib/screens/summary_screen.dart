...
class _SummaryScreenState extends State<SummaryScreen>
  ...
  String _fmtP(double w) {
    switch (_powerUnit) {
      case 'mW':
        final v = w * 1000;
        return '${v.toStringAsFixed(v >= 100 ? 0 : 1)} mW';
      case 'kW':
        final v = w / 1000;
        return '${v.toStringAsFixed(3)} kW';
      default:
        return '${w.toStringAsFixed(w >= 100 ? 0 : 1)} W';
    }
  }
  ...
  List<_DaySummary> _buildCombinedDailySummaries(
    List<List<EnergyData>> unitsData,
  ) {
    final perUnit = unitsData.map(_buildDailySummaries).toList();
    final allDates = <String>{
      for (final u in perUnit)
        for (final d in u) d.date,
    };
  ...
  List<_MonthSummary> _buildMonthlySummariesFromDaily(
    List<_DaySummary> dailies,
  ) {
    ...
    return map.entries.map((e) {
      ...
      final peakPower = days.fold(
        0.0,
        (m, d) => d.peakPowerW > m ? d.peakPowerW : m,
      );
      final avgPF = days.isEmpty
          ? 0.0
          : days.fold(0.0, (s, d) => s + d.avgPowerFactor) / days.length;
      final avgPower = days.isEmpty
          ? 0.0
          : days.fold(0.0, (s, d) => s + d.avgPowerW) / days.length;
      final avgVoltage = days.isEmpty
          ? 0.0
          : days.fold(0.0, (s, d) => s + d.avgVoltageV) / days.length;
      final avgCurrent = days.isEmpty
          ? 0.0
          : days.fold(0.0, (s, d) => s + d.avgCurrentA) / days.length;
    ...
    try {
      ...
      if (_selectedUnit == _allUnitsKey) {
        ...
        final allLists = await Future.wait(
          allUnits.map((u) => _fetchUnit(u, since)),
        );
  ...
  Future<void> _loadUnitStatus() async {
    final status = await FirestoreLogService.instance.getUnitStatus(
      _selectedUnit,
    );
    if (!mounted || status == null) return;
    final ts =
        (status['updatedAt'] as Timestamp?)?.toDate().toLocal() ??
        (status['timestamp'] as Timestamp?)?.toDate().toLocal();
    final isActive = ts != null && DateTime.now().difference(ts).inMinutes < 10;
    setState(() {
      _unitLastSeen = ts;
  ...
  List<_DaySummary> _buildDailySummaries(List<EnergyData> data) {
    ...
    return map.entries.map((e) {
      ...
      final avgPF = pts.isEmpty
          ? 0.0
          : pts.fold(0.0, (s, p) => s + p.powerFactor) / pts.length;
      final avgPower = pts.isEmpty
          ? 0.0
          : pts.fold(0.0, (s, p) => s + p.power) / pts.length;
      final avgVoltage = pts.isEmpty
          ? 0.0
          : pts.fold(0.0, (s, p) => s + p.voltage) / pts.length;
      final avgCurrent = pts.isEmpty
          ? 0.0
          : pts.fold(0.0, (s, p) => s + p.current) / pts.length;
      return _DaySummary(
        date: e.key,
        energyMwh: totalEnergy,
        peakPowerW: peakPower,
        ...
        children: [
          ...
          Expanded(
            child: _loading
                ...
                : TabBarView(
                    controller: _tab,
                    children: [_buildDailyTab(), _buildMonthlyTab()],
                  ),
          ),
        ],
      ),
  ...
  Widget _buildHeader() {
    return LayoutBuilder(
      builder: (context, bc) {
        ...
        final deviceLabel = isAll
            ? 'Toutes les unités'
            : (_deviceLabel[_selectedUnit] ?? _selectedUnit);
        return Container(
          ...
          child: Column(
            children: [
              ...
              _buildKpiRow([
                ...
                _KpiData(
                  'Moy. journalière',
                  _fmtE(totalEnergy / _dailySummaries.length),
  ...
  Widget _buildDailyChart() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Consommation journalière (14 derniers jours)',
            style: TextStyle(
              color: _c.textPri,
              fontWeight: FontWeight.bold,
              fontSize: 15,
          ...
          child: Column(
            children: [
              _buildKpiRow([
                ...
                _KpiData(
                  'Moy. mensuelle',
                  _fmtE(totalEnergy / _monthlySummaries.length),
                  const Color(0xFFFF6B8A),
                  Icons.trending_up,
                ),
              ]),
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              SizedBox(
                width: 90,
                child: Text(c2, style: style, textAlign: TextAlign.right),
              ),
              SizedBox(
                width: 85,
                child: Text(c3, style: style, textAlign: TextAlign.right),
              ),
              SizedBox(
                width: 80,
                child: Text(c4, style: style, textAlign: TextAlign.right),
              ),
              SizedBox(
                width: 80,
                child: Text(c5, style: style, textAlign: TextAlign.right),
              ),
              SizedBox(
                width: 60,
                child: Text(c6, style: style, textAlign: TextAlign.right),
              ),
              SizedBox(
                width: 85,
                child: Text(c7, style: style, textAlign: TextAlign.right),
              ),
            ],
          ),
        ),
